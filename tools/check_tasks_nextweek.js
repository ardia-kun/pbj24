...
if (fs.existsSync('data/daftar-tugas.csv')) {
  ...
  data = parseCSV(daftarText).map(t => ({...t, _source: 'daftar'}));
} else {
  const depanText = fs.readFileSync('data/minggu-depan.csv', 'utf8');
  data = parseCSV(depanText).map(t => ({...t, _source: 'depan'}));
}
...
data.forEach(t => {
  ...
  if (parsed) {
    if (isNextWeek(parsed, today)) tasksNextWeek.push({t, parsed});
  } else {
    tasksNextWeek.push({t, parsed: null});
  }
});

console.log('Tanggal hari ini:', today.toISOString().slice(0,10));
if (tasksNextWeek.length === 0) console.log('Tidak ada tugas untuk minggu depan.');
else {
  console.log('Tugas yang dianggap untuk minggu depan:');
  tasksNextWeek.forEach(({t, parsed}, i) => {
